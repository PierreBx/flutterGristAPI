...
#doc_header(
  "API Reference",
  subtitle: "Public API Documentation for flutter_grist_widgets",
  version: "0.3.0"
)
...
// Store hash in Grist Users table
...
```dart
// Get GristService (doesn't rebuild on changes)
...
// Watch AuthProvider (rebuilds on changes)
final auth = context.watch<AuthProvider>();
if (auth.isAuthenticated) {
  // User is logged in
}

// Get config (doesn't rebuild)
...
#info_box(type: "info")[
  This API reference covers the public API. Internal implementation details are subject to change. Always use the public API exports from `package:flutter_grist_widgets/flutter_grist_widgets.dart`.
]
