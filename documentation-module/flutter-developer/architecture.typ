...
#import "../common/architecture.typ": architecture_overview

#apply_standard_styles()

#doc_header(
  "Library Architecture",
  subtitle: "Understanding FlutterGristAPI Internals",
  version: "0.3.0"
)

// Include the system-level architecture overview
...
```dart
// Each validator type is a strategy
class FieldValidator {
  String? validate(dynamic value) {
    switch (type) {
...
// 2. Register in page type factory
...
```dart
// Add case in FieldValidator.validate()
case 'custom':
...
```dart
// Add case in GristFormWidget._buildField()
case 'custom':
...
#info_box(type: "info")[
  For extending the library, see `extending.typ`. For API details, see `api-reference.typ`.
]
