// Common Commands and API Reference for Grist Managers
...
#info_box(type: "warning")[
  *Deletion is permanent*: Consider deactivating users instead of deleting them to preserve data integrity and history.
]
...
#info_box(type: "info")[
  *Error Handling*: Always check the HTTP status code. On error, the response body usually contains a helpful error message in JSON format.
]
...
#info_box(type: "success")[
  *Pro Tip*: Save frequently used commands as shell functions in your `.bashrc` or create wrapper scripts for common operations.
]
