// Data Operations for Grist Managers
...
#info_box(type: "info")[
  *Batch imports*: For large datasets, split into batches of 100-500 records per API request for optimal performance and to avoid timeouts.
]
...
#info_box(type: "warning")[
  *Schema compatibility*: Destination table must have compatible columns (same names and types) for paste to work correctly.
]
...
#info_box(type: "info")[
  *Filtering before export*: Apply filters to export only specific records. For example, filter `active = true` to export only active users.
]
...
#info_box(type: "success")[
  *Best practice*: Export entire document as SQLite regularly for backups. Store in secure, versioned location.
]
...
#info_box(type: "danger")[
  *Permanent deletion*: Deleted records cannot be recovered unless you have a backup. Consider using an `active` flag instead of deleting.
]
...
#info_box(type: "info")[
  *Validation columns*: Add hidden validation columns to check data integrity. Filter by `is_complete = false` to find invalid records.
]
...
#table(
  ...
  [Export and check for duplicates],

  [Valid email format],
  [Validation formula column],
  [Filter where `email_valid = false`],

  [Active users have login info],
  [Formula: `not $active or ($password_hash and $email)`],
  [Filter where validation fails],

  [Prices are positive],
  [Application/formula validation],
  [Filter where `price <= 0`],

  [References exist],
  [Use Reference columns],
  [Check for blank references],
)
...
#info_box(type: "info")[
  *Grist performance*: Self-hosted Grist can handle larger datasets better with proper hardware. Consider hosting requirements for large databases.
]
...
#info_box(type: "success")[
  *Backup golden rule*: Test your backups by restoring them. A backup you can't restore is worthless.
]
