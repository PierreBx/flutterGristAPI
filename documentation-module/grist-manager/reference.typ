// Complete Reference Guide for Grist Managers
...
#table(
  ...
  [Text],
  [✓],
  [User's email address for login. Must be unique. Used as username.],

  [`password_hash`],
  [Text],
  [✓],
  [Bcrypt or Argon2 hash of user's password. Never store plain text. FlutterGristAPI handles hashing automatically.],

  [`role`],
  [Text or Choice],
  [✓],
  [User's role for authorization. Common values: 'admin', 'manager', 'user'. Case-sensitive.],

  [`active`],
  [Toggle],
  [✓],
  [Whether user account is active. Must be `true` for login to work. Use `false` to disable accounts.],
)
...
#table(
  ...
  [`full_name`],
  [Text],
  [Display name for UI. More friendly than email.],

  [`created_at`],
  [DateTime],
  [Account creation timestamp. Useful for analytics and auditing.],

  [`updated_at`],
  [DateTime],
  [Last profile update. Track when user info changes.],

  [`last_login`],
  [DateTime],
  [Most recent successful login. Monitor inactive accounts.],

  [`phone`],
  [Text],
  [Contact phone number. Optional contact method.],

  [`profile_picture`],
  [Attachments],
  [User avatar image. Enhances UI experience.],

  [`email_verified`],
  [Toggle],
  [Email verification status. Security feature.],

  [`two_factor_enabled`],
  [Toggle],
  [2FA status. Additional security layer.],

  [`notes`],
  [Text],
  [Admin notes about user. Not visible to user.],
)
...
#info_box(type: "danger")[
  *Security Critical*: Never store passwords as plain text or use weak hashing like MD5/SHA-1. Always use bcrypt or Argon2 with appropriate cost factors.
]
...
#table(
  ...
  [Use filters],
  [Fetch only needed records: `?filter={"active":[true]}`],

  [Limit results],
  [Add limit parameter: `?limit=100`],

  [Cache frequently accessed data],
  [Cache in app memory/storage, refresh periodically],

  [Batch operations],
  [Create/update/delete multiple records in one API call],

  [Archive old data],
  [Move old records to archive tables],

  [Convert formulas to data],
  [Freeze calculated values for better performance],

  [Use appropriate data types],
  [Toggle, not Text for booleans. Numeric, not Text for numbers.],

  [Minimize complex formulas],
  [Move complex logic to application when possible],
)
...
#table(
  ...
  [API Key],
  [Authentication token for accessing Grist API programmatically],

  [bcrypt],
  [Password hashing algorithm. Industry standard for secure password storage],

  [Choice Column],
  [Column type with predefined list of allowed values (dropdown)],

  [Document],
  [Top-level Grist container, similar to a database. Contains multiple tables],

  [Document ID],
  [Unique identifier for a Grist document, found in URL],

  [Field],
  [A value in a record. Synonym for column value],

  [Formula Column],
  [Column with calculated values based on Python formula],

  [Password Hash],
  [One-way encrypted representation of a password. Cannot be reversed],

  [Record],
  [A single row in a table. Contains values for each column],

  [Record ID],
  [Unique numeric identifier for a record. Auto-generated by Grist],

  [Reference Column],
  [Column that links to records in another table (foreign key)],

  [Role],
  [User permission level (e.g., admin, manager, user)],

  [Schema],
  [Structure of database: tables, columns, types, relationships],

  [Table],
  [Collection of records with defined columns. Similar to database table],

  [Toggle Column],
  [Boolean column type (true/false, yes/no)],
)
...
#info_box(type: "success")[
  *Bookmark this reference*: Keep it handy for quick lookups while managing your Grist databases!
]
