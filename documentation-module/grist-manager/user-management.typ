// User Management for Grist Managers
...
#info_box(type: "danger")[
  *Critical Security Rule*: NEVER store plain-text passwords in the `password_hash` column. Always use properly hashed passwords. FlutterGristAPI handles this automatically when creating users through the API.
]
...
#table(
  ...
  [`full_name`],
  [Text],
  [User's display name for the application UI],
  ...
  [`profile_picture`],
  [Attachments],
  [User's avatar or profile photo],
  ...
  [`notes`],
  [Text],
  [Admin notes about the user (not visible to user)],
)
...
#info_box(type: "warning")[
  *Manual password hashing*: When adding users manually via the web interface, you must hash passwords yourself before entering them. See the "Password Hashing" section below.
]
...
#info_box(type: "success")[
  *Automatic hashing*: FlutterGristAPI automatically hashes passwords using bcrypt when you call `createUser()`. You provide the plain-text password, and the library handles hashing before storing it in Grist.
]
...
#info_box(type: "danger")[
  *Never store plain-text passwords*. Even in development/testing environments, use proper password hashing. It's a fundamental security requirement.
]
...
#info_box(type: "warning")[
  *Testing only*: Online hash generators are fine for development but never use them for production passwords. Use server-side hashing in your application.
]
...
#info_box(type: "info")[
  *Grist Access Rules* can also restrict data access at the database level based on roles. See the "Access Control" section below.
]
...
#info_box(type: "warning")[
  *Changing email*: Since email is used for login, changing it will affect how the user logs in. Communicate this to the user before changing.
]
...
#info_box(type: "info")[
  *Password reset flow*: Typically, password resets should be handled by your application, not manually by Grist Managers. The app should generate a reset token, email it to the user, and let them set a new password through the app.
]
...
#info_box(type: "warning")[
  *Hash passwords first*: Before importing, ensure all passwords in the CSV are already hashed. Never import plain-text passwords.
]
...
#info_box(type: "warning")[
  *Deleting users*: This removes the record permanently. If any other tables reference this user (e.g., Orders.user_id), those references will break. Consider deactivating instead.
]
