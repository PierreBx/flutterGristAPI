// Troubleshooting Guide - FlutterGristAPI
// Common issues and solutions for DevOps operations
...
#troubleshooting_table((
  (
    ...
    solution: "Check logs with `docker logs container_name`. Common causes: missing environment variables, port conflicts, permission issues.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Check processes with `docker top container_name`. Review application logs. Consider resource limits.",
    priority: "medium"
  ),
  (
...
#troubleshooting_table((
  (
    issue: "http://localhost:8484 not responding",
    solution: "Check if Grist is running: `docker ps | grep grist`. Check firewall. Try `docker-compose restart grist`.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    issue: "Grist data disappeared",
    solution: "Check if volume mounted correctly: `docker inspect grist_server | grep -A 5 Mounts`. Restore from backup if needed.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    issue: "Docker build fails or hangs",
    solution: "Clean build cache: `docker builder prune`. Try: `docker-compose build --no-cache`",
    priority: "medium"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Check Flutter version matches. Verify dependencies: `docker exec flutter_dev flutter pub get`",
    priority: "medium"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Clean Docker: `docker system prune -a`. Remove old images/containers. Free host disk space.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Check if Docker running: `systemctl status docker` (Linux) or check Docker Desktop. Restart if needed.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Check they're on same network: `docker network inspect grist-network`. Recreate network if needed.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Verify .env in project root. Check file permissions. Restart services after changes.",
    priority: "high"
  ),
  (
...
#troubleshooting_table((
  (
    ...
    solution: "Check disk space. Verify permissions. Check paths are correct. Review script syntax.",
    priority: "medium"
  ),
  (
...
#troubleshooting_table((
  (
    issue: "Overall system slowness",
    solution: "Check host resources: `top`, `htop`. Check Docker stats. Restart services. Consider scaling up.",
    priority: "medium"
  ),
  (
