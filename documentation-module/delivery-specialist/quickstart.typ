// Quickstart Guide for Delivery Specialists
...
#info_box(type: "warning")[
  *Security Note*: Never commit `credentials.yml` to version control. It's already in `.gitignore`, but double-check that sensitive data stays local.
]
...
#info_box(type: "info")[
  *First Time?* The UI will be empty. That's normal! You'll deploy your pipeline in the next phase.
]
...
#info_box(type: "success")[
  *Congratulations!* You now have a fully automated CI/CD pipeline. Every commit will be tested, and merges to main will automatically deploy to production.
]
