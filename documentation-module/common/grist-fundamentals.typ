// Grist fundamentals and basic concepts
...
#let grist_basics() = [
  ...
  *Delete a record:*
  ...
  #info_box(type: "warning")[
    *Security Warning:* Never store plain-text passwords in Grist. Always use hashed passwords (bcrypt, Argon2, etc.). The FlutterGristAPI library handles password hashing automatically during user creation.
  ]
...
#let grist_connection_config() = [
  ...
  #info_box(type: "info")[
    *Environment Variables:* Use `${VAR_NAME}` syntax in YAML to reference environment variables. This keeps sensitive data out of version control.
  ]
]
